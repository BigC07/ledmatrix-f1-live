...
class F1Renderer:
    ...
    def render_race_header(self, race: Dict) -> Image.Image:
        """Name card that opens the race section.

        Local patch (repo patches/patch_f1_race_rows.py). Same furniture as the
    ...
    def _race_position_font(self):
        ...
        Local patch (repo patches/patch_f1_race_rows.py). One tier above the
        rest of the row so the position reads first. Cached: _load_font() hits
        the filesystem and this is called once per card.

        Only on short panels. At >=48px the renderer's own tier already picks a
        large face and overriding it here would shrink the number.
        """
        cached = getattr(self, "_race_pos_font_cache", None)
        if cached is not None:
            return cached
        font = self.fonts["position"]
        if not self.is_tall:
            try:
                bigger = self._load_font("9x15.bdf", 15)
                if bigger is not None:
                    font = bigger
            except Exception:
                self.logger.debug("9x15.bdf unavailable; race positions stay at "
                                  "the default size")
    ...
    def render_race_row(self, entry: Dict, live: bool = False) -> Image.Image:
        ...
        call into _render_driver_row(): that one puts the lap time on line 1
        beside the name, which squeezes the name down to the 3-letter code and
        is what qualifying wants. Here the time drops to line 2, which leaves
        line 1 wide enough for the surname.

        Line 1:  P<pos>  SURNAME
        Line 2:  time or gap (left)        places gained/lost (right, by logo)
        """
        img = Image.new("RGBA", (self.display_width, self.display_height), (0, 0, 0, 255))
        draw = ImageDraw.Draw(img)
        # Disable anti-aliasing, as elsewhere in this renderer: pixel fonts go
        # muddy on a 1:1 LED matrix otherwise.
        draw.fontmode = "1"
        ...
        # from a different lap and means nothing to a reader.
